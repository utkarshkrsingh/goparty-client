...
export default function JoinRoom({ setView }: PageProps) {
    ...
    const handleLogin = (e: React.FormEvent) => {
        e.preventDefault();
        console.log('Logging in with: ', { roomCode });
    ...
    return (
        <div className={`w-full max-w-sm text-center animate-fade-in ${darkMode ? "bg-gray-900 border-gray-800" : "bg-white border-gray-200"} p-8 rounded-xl shadow-2xl border`}>
            <h2 className={`text-3xl font-bold ${darkMode ? "text-white" : "text-gray-900"} mb-6`}>Join Room</h2>
            <form onSubmit={handleLogin} className="space-y-6">
                <div className="relative">
                    <FaCode className={`absolute left-3 top-1/2 -translate-y-1/2 ${darkMode ? "text-gray-500" : "text-gray-400"}`} />
                    <input
                        type="text"
                        value={roomCode}
